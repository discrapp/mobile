...
describe('stripeFees', async () => {
  describe('calculateStripeFee', async () => {
    ...
    it('returns 0 for negative reward', async () => {
      expect(calculateStripeFee(-10)).toBe(0);
    });

    it('calculates fee for $10 reward', async () => {
    ...
    it('calculates fee for $20 reward', async () => {
    ...
    it('calculates fee for $50 reward', async () => {
  ...
  describe('calculateTotalWithFee', async () => {
    ...
    it('calculates total for $10 reward', async () => {
      const total = calculateTotalWithFee(10);
      expect(total).toBeGreaterThan(10);
      expect(total).toBeCloseTo(10.61, 1);
    });

    it('calculates total for $20 reward', async () => {
  ...
  describe('formatFeePreview', async () => {
    ...
    it('returns empty string for negative reward', async () => {
      expect(formatFeePreview(-10)).toBe('');
    });

    it('formats fee preview for $10 reward', async () => {
    ...
    it('formats fee preview for $20 reward', async () => {
      const preview = formatFeePreview(20);
      expect(preview).toMatch(/\$[\d.]+\s+with card/);
      expect(preview).toContain('processing fee');
    });

    it('includes total and fee amounts', async () => {
      const preview = formatFeePreview(10);
  ...
  describe('formatFeeHint', async () => {
    ...
    it('formats fee hint for $10 reward', async () => {
      const hint = formatFeeHint(10);
      expect(hint).toContain('Card:');
      expect(hint).toContain('fee');
    });

    it('formats fee hint for $20 reward', async () => {
